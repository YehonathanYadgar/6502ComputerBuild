// RAM/EEPROM debug monitor.
// The Arduino drives PHI2 manually and samples the 6502 data bus plus key
// control signals while the clock is high.

const byte DATA[] = {3, 4, 5, 6, 7, 8, 9, 10};  // Arduino pins wired to D0-D7.
...
void printStatusOnly() {
  ...
  printDebugLine(data, ce, resb, a15, ramCS, ramWE, ramOE, cpuRW, false);
}
...
void makeClockPulse() {
  ...
  delayMicroseconds(10);  // Let address, data, and control signals settle.
  ...
  printDebugLine(data, ce, resb, a15, ramCS, ramWE, ramOE, cpuRW, true);
}
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    // The "rc" command arrives as two serial characters: first r, then c.
