...
### EEPROM Chip (ROM):
* Gets an Input from the cpu's address buss.
* Looks up the stores data at that address.
* Outputs that data through the data buss.
...
### Register
A register is a small and very fast fast storage unit inside the CPU, used to Temporarily hold data like the.

### Interface chip
Can consistently output or listen for an input to or from an external device.
allows the CPU to read inputs from external devices and control outputs such as LEDs or an LCD. Its ports can be configured as inputs or outputs through internal registers.(To add an explnation under this img)
...
#### RTS
When RTS is called the CPU increments SP by one and reads the little endian byte of the return address.  and then increment's by one again and reads the big endian byte. combines them and adds one ( Becuse`JSR` saves an address **one byte before where execution should continue**, so `RTS` adds 1 to get the correct return address. )
